...
# QR payload + segno parameters that reproduce the
# gf180mcu_ws_ip__id template's QR matrix exactly. The chip ships with
# a placeholder ID of "00000000" (the IP source has not been wired up
# to per-project IDs yet — see commit f6c3543 in
# mithro/gf180mcu-project-template). probe_segno_match.py confirmed
# segno produces a byte-identical 21x21 matrix at error="H", mask=3,
# mode="numeric" — so we can regenerate the QR cleanly here without
# falling back on a raster crop of the GDS render.
WSIP_QR_DATA = "00000000"
WSIP_QR_ERROR = "H"
WSIP_QR_MASK = 3
WSIP_QR_MODE = "numeric"
...
def render(cell_name: str, pads: list[Pad], die_bb: tuple[float, float, float, float],
    ...
    # (small), QR image. The QR is generated by segno using the same
    # payload + parameters as the gf180mcu_ws_ip__id template cell, so
    # the panel's matrix is byte-identical to the QR drawn in the
    # chip's own bottom-left corner — verified by probe_segno_match.py
    # against the matrix extracted directly from the IP's GDS.
    ...
    _draw_qr_on_ax(ax_qr, WSIP_QR_DATA,
                   error=WSIP_QR_ERROR, mask=WSIP_QR_MASK,
